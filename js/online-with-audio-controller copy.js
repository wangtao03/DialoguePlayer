new Vue({
    ...
    created() {                                                                         //vue创建时建立ws连接
        ...
        this.reconnectInterval = setInterval(this.checkWebSocketStatus, 1000);
        // 组件创建时导入lyricsArray  
    ...
    methods: {
        ...
        connectWebSocket() {
            ...
            this.webSocket.onmessage = (event) => {     //解析服务器消息
                const data = JSON.parse(event.data);
                this.status = data.status.toLowerCase();
                this.duration = data.duration;
                if (this.duration >= 3600000) {
                    this.sendCommand("stop");       //超过1消失自动停止
                }
                this.updateTime();
            };
        },
        disconnectWebSocket() { //关闭
            if (this.webSocket) {
        ...
        updataAudio() {
            if (this.force) {
                this.force = false;
                this.audio.currentTime = this.duration / 1000;
                // if (this.status == 'running') this.audio.play();
            }
            if (this.audio.paused && this.status == 'running') {
                this.audio.currentTime = this.duration / 1000;
                this.audio.play();
                return;
            }
        ...
        sendCommand(message) {
            if (this.webSocket.readyState === WebSocket.OPEN) {
                this.webSocket.send(JSON.stringify({ command: message }));
                this.force = true;
            } else {
                switch (message) {
                    case 'start':
